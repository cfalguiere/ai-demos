...
! tmux has-session -t airflowdb 2>/dev/null && {
  ...
  tmux new -d -s airflowdb bash -c "conda activate python36; airflow initdb" 
...
! tmux has-session -t airflowws 2>/dev/null && {
  ...
  tmux new -d -s mlflowui bash -c "conda activate python36; airflow webserver -p 8080" &
}

# start the scheduler
! tmux has-session -t airflowscheduler 2>/dev/null && {
  echo "=== starting airflow scheduler"
  tmux new -d -s mlflowui bash -c "conda activate python36; airflow scheduler" &
}
