...
! tmux has-session -t airflowws 2>/dev/null && {
  ...
  tmux new -d -s mlflowui bash -c "source /etc/profile.d/conda.sh; conda activate python36; cd /opt/airflow; airflow webserver -p 8080" &
}

# start the scheduler
! tmux has-session -t airflowscheduler 2>/dev/null && {
  echo "=== starting airflow scheduler"
  tmux new -d -s mlflowui bash -c "source /etc/profile.d/conda.sh; conda activate python36; cd /opt/airflow; airflow scheduler" &
}
