...
! tmux has-session -t jupyter  2>/dev/null && {
  ...
  tmux new -d -s jupyter bash -c "source /etc/profile.d/conda.sh; conda activate python36; source env-setup/setenv.sh; cd /opt/jupyter; jupyter notebook | tee ${AIDEMOS_TRACKING_DIR}/jupyter.out" &
}
